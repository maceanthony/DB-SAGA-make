...
void do_slookup( CHAR_DATA *ch, char *argument )
{
	...
	ch_printf( ch, "&cMana: &w%d  &cFocus: &w%d  &cCanCharge: &w%d\n\r",
		skill->min_mana, skill->focus, skill->canCharge);
...
/*	    char bitvector[MAX_INPUT_LENGTH];	*/
	...
	*/
	...
	/* Narn is dumb, ch changed to victim by Saiyr */
	    if (  skill_table[sn]->min_level[victim->class] == 0
	    || skill_table[sn]->skill_level[victim->class] == 0)
 	        continue;

	    if ( skill_table[sn]->name
            && ( victim->exp >= skill_table[sn]->skill_level[victim->class]
                      || value == 0 ) )
	    {
...
void learn_from_success( CHAR_DATA *ch, int sn )
{
	...
	if ( ch->pcdata->learned[sn] < adept )
	{
		...
		if ( percent >= chance )
			learn = (double)get_curr_int(ch) / number_range(70, 120);
		else
			learn = (double)get_curr_int(ch) / number_range(120, 150);
...
void learn_from_failure( CHAR_DATA *ch, int sn )
{
	...
	if ( IS_NPC(ch) || ch->pcdata->learned[sn] <= 0 )
		return;
	adept = GET_ADEPT(ch, sn);
	learn = (double)get_curr_int(ch) / number_range(50, 70);
...
{
   bool check = FALSE;
   if ( IS_NPC(ch) && percent < 100 ) check = TRUE;
