...
/* clean out then fill up RTG shops */
void shop_inventory_update( void )
{
    ...
    for ( mob = first_char; mob; mob = mob->next )
	{
		...
		if(xIS_SET(mob->act, ACT_RTG_SHOP))
		{
			for ( obj = mob->first_carrying; obj; obj = obj_next )
			{
				...
				if ( !obj_next )
					obj_next = mob->first_carrying;
			}

			howManyItems = number_range(10, 20);
			for (i = 1; i <= howManyItems; i++)
				obj_to_char(generate_item(mob->exp,mob->exp), mob);
		}
	}
...
int get_cost( CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj, bool fBuy )
{
    ...
    if ( ch->gold > (get_true_rank(ch) * get_true_rank(ch) * 100000) )
	richcustomer = TRUE;
    else
	richcustomer = FALSE;
...
//	cost = (int) (cost * (98 + get_true_rank(ch))) / 100;

        switch(ch->race) /* racism... should compare against shopkeeper's race */
        {
          case(0): cost /=1; break;/* saiyan */
          case(1): cost /=1; break; /* human */
          case(2):  cost /=1; break;/* halfbreed */
          case(3):  cost /=1; break;/* namek */
	  case(4):  cost /=1; break;/* android */
	  case(5):  cost /=1; break;/* icer */
          case(6):  cost /=1; break;/* bio-android */
          case(7):  cost /=1; break;/* kaio */
          case(8):  cost /=1; break;/* demon */
          case(9):  cost /=1; break;/* wizard */
        }
...
//	OBJ_DATA *obj2;
	...
	for ( itype = 0; itype < MAX_TRADE; itype++ )
	{
	    ...
	    {
			/* Forcing 1% sell ratio cause dem armors get some
			 * nice cost values now -Goku 09.28.04 */
			cost = obj->cost * 0.01;
/*
		cost = (int) (obj->cost
...
*/
	...
	ch->gold     -= cost;
	/* bost the economy instead of the keeper holding the zeni.  the
	 * areas "economy* value will now be very important! -Goku 09.28.04 */
	...
	if ( keeper->gold > maxgold )
	{
	    ...
	    act( AT_ACTION, "$n puts some zeni into a large safe.", keeper, NULL, NULL, TO_ROOM );
	}
...
/*      bool listall; */
                ...
                obj = obj->next_content )
        {
            ...
            {
            /* Reformated for big numbers.  Thanks Karn and Gehenom! */
                if ( !found )
                {
                    found = TRUE;
                    send_to_pager( "&w[ #   &WPL Needed            Price&w] &WItem&w\n\r", ch );
                    send_to_pager( "&w[--+---------------+------------] ----&w\n\r", ch );
                }
            	...
            	num++;
                pager_printf( ch, "&w[&W%2d|%15s|", num, num_punct_ld(obj->level));
                pager_printf( ch, "%12s&w]&Y %s.\n\r",
                    num_punct(cost), obj->short_descr );
            }
        }
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !economy_has(keeper->in_room->area, cost) )
    {
    ...
    sprintf( buf, "You sell $p for %d zeni.",
	cost );
...
void do_value( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "$n tells you 'I'll give you %d zeni for $p.'", cost );
                 ...
                 char *arg, int maxgold, char *fixstr, char*fixstr2 )
{
   ...
   else
   {
      ...
      sprintf( buf, "$N charges you %d zeni to %s $p.",
          cost, fixstr );
      ...
      if ( keeper->gold > maxgold )
      {
          boost_economy( keeper->in_room->area, keeper->gold - maxgold/2 );
          keeper->gold = maxgold/2;
          act( AT_ACTION, "$n puts some zeni into a large safe.", keeper,
		NULL, NULL, TO_ROOM );
      }
...
void appraise_all( CHAR_DATA *ch, CHAR_DATA *keeper, char *fixstr )
{
    ...
    for ( obj = ch->first_carrying; obj != NULL ; obj = obj->next_content )
    {
        ...
        ||   obj->item_type == ITEM_STAFF ) ) {
            ...
            {
            sprintf( buf,
            "$N tells you, 'It will cost %d zeni to %s %s'",
    ...
    if ( total > 0 )
    {
       send_to_char ("\n\r", ch);
       sprintf( buf,
          "$N tells you, 'It will cost %d zeni in total.'",
          total );
       act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
       strcpy( pbuf,
...
void do_appraise( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
       "$N tells you, 'It will cost %d zeni to %s that...'", cost,
       fixstr );
    act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
    if ( cost > ch->gold )
      act( AT_TELL, "$N tells you, 'Which I see you can't afford.'", ch,
