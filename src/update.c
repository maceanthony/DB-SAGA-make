...
void transform_makeo( CHAR_DATA *ch )
{
	  ...
	  return;
        ...
        act( AT_RED, "You yell out as the evil energy radiating from the Makeo Star increases your power!", ch, NULL, NULL, TO_CHAR );
        act( AT_RED, "Sinister light from the Makeo Star shines down upon $n.", ch, NULL, NULL, TO_NOTVICT );
        act( AT_RED, "$n yells out as the evil energy radiating from the Makeo Star increases $s power!", ch, NULL, NULL, TO_NOTVICT );
...
void transform_golden_oozaru( CHAR_DATA *ch )
{
        ...
        act( AT_WHITE, "$n gazes up at the full moon and $e begins to change.", ch, NULL, NULL, TO_NOTVICT );
        act( AT_YELLOW, "$n lets out an earth shattering howl as $e transforms into a Golden Oozaru!", ch, NULL, NULL, TO_NOTVICT );
...
void hair_growth( CHAR_DATA * ch )
{
   ...
   case STYLE_MEDIUM_BANGS:
      ...
      break;
   case STYLE_LONG_BANGS: case STYLE_PONYTAIL: case STYLE_FLOWING:
...
/*  PL high enough to gain a train point */
	...
	if (is_android(ch))
	ch_printf( ch, "You gained 5 additional hardware upgrade points!\n\r" );
	else
	ch_printf( ch, "You gained 5 additional training points!\n\r" );
	ch->train += 5;
	ch->max_train += 1;
    }
...
/*  PL high enough to gain more energy */
    ...
    if (ch->exp >= (pow(ch->max_energy,2.45) * 300)
    	&& ch->max_mana < 99999 )
    {
	set_char_color( AT_LBLUE + AT_BLINK, ch );
	ch_printf( ch, "You gained some more energy!\n\r" );

	energygain = (double)((double)get_curr_con(ch)/100 + 1) * (pow(get_curr_con(ch), 1.15)) + number_range(5, 15);
	ch->mana += energygain;
...
int hit_gain( CHAR_DATA *ch )
{
	...
	case POS_MORTAL:   gain = number_range(1,3);
	case POS_INCAP:    gain = number_range(1,3);
	case POS_STUNNED:  gain = number_range(1,3);
	case POS_SLEEPING: gain *= 2.0;	break;
	case POS_RESTING:  gain *= 1.25;	break;
	}
...
void summon_update( void )
{
    ...
    {
	sprintf(buf,"&YThe column of bright, golden energy grows larger, twisting and weaving through the sky as it takes shape.");
        echo_to_room(AT_YELLOW,summon_room,buf);
	...
	sprintf(buf,"&gThe light subsides, revealing a collossal, green serpent dragon floating ominously in the sky above; looking down at you.");
    ...
    {
	sprintf(buf,"&CThe Eternal Dragon 'Shenron' says in a commanding voice 'I am the Eternal Dragon. I will grant you one wish, and one wish only. Now, tell me what it is you wish me to grant you.'");
    ...
    {
	sprintf(buf,"&CThe Eternal Dragon 'Shenron' says in a commanding voice 'Your wish has been granted. And now, I bid you farewell.");
	echo_to_room(AT_LBLUE,summon_room,buf);
	sprintf(buf,"&YShenron turns bright gold and vanishes, the dragonballs suddenly blazing with incredible golden energy.");
	echo_to_room(AT_YELLOW,summon_room,buf);
    ...
    {
	sprintf(buf,"&YThe dragonballs suddenly shoot straight up into the sky and stop. The dragonballs flash once, then rocket off in random directions; disappearing into the far distance.");
	echo_to_area(summon_area,buf);
...
void mystic_check( void )
{
	...
	{
	  act( AT_WHITE, "You finish dancing infront of $N, and sit down on the ground. You then throw your hands out infront of $N and say \"BLUAGH\" as you start into the second half of the mystic training.", ch->teaching, NULL, ch, TO_CHAR );
	  act( AT_WHITE, "$n finishes dancing infront of you, and sits down on the ground. $n then throws $s hands out infront of you and says \"BLUAGH\" as $e starts into the second half of the mystic training.", ch->teaching, NULL, ch, TO_VICT );
 	  act( AT_WHITE, "$n finishes dancing infront of $N, and sits down on the ground. $n then throws $s hands out infront of $N and says \"BLUAGH\" as $e starts into the second half of the mystic training.", ch->teaching, NULL, ch, TO_NOTVICT );
	...
	{
	  act( AT_WHITE, "You continue dancing in a large circle around $N and chanting random gibberish.", ch->teaching, NULL, ch, TO_CHAR );
	  act( AT_WHITE, "$n continues dancing in a large circle around you and chanting random gibberish.", ch->teaching, NULL, ch, TO_VICT );
	  act( AT_WHITE, "$n continues dancing in a large circle around $N and chanting random gibberish.", ch->teaching, NULL, ch, TO_NOTVICT );
	  continue;
	}
	else if( ch->mysticlearn < 15 && ch->mysticlearn > 0 )
	{
	  act( AT_WHITE, "You continue sitting infront of $N with your palms facing $e, as you concentrate on releasing $s hidden powers.", ch->teaching, NULL, ch, TO_CHAR );
	  act( AT_WHITE, "$n continues sitting infront of you with $s palms facing you, as $e concentrates on releasing your hidden powers.", ch->teaching, NULL, ch, TO_VICT );
	  act( AT_WHITE, "$n continues sitting infront of $N with $s palms facing $N, as $e concentrates on releasing $N's hidden powers.", ch->teaching, NULL, ch, TO_NOTVICT );
	  continue;
	}
	else if( ch->mysticlearn == 0 && ch->tmystic == 3 )
	{
	  ...
	  act( AT_WHITE, "$n suddenly tells you that you are done. You have successfully learned mystic.", ch->teaching, NULL, ch, TO_VICT );
	  act( AT_WHITE, "You tell $N that you are done. You have successfully taught $N mystic.", ch->teaching, NULL, ch, TO_CHAR );
	  act( AT_WHITE, "$n suddenly tells $N that $e is done. $N has successfully learned mystic.", ch->teaching, NULL, ch, TO_NOTVICT );
 ...
 * Will take care of other similar rank systems as they are
 * added/needed. -Karma
 */
...
void rank_update( void )
{
	...
	if( IS_NPC(ch) )
	  continue;
	if( ch->kairank < 1 && ch->demonrank < 1 )
	  continue;
	if( IS_IMMORTAL(ch) )
	  continue;
	for( a = 0; a < 6; a++ )
	...
	if( !found && ch->kairank > 0 )
	{
	  bug("%s has a kai rank and is not on the global list. Removing rank.",ch->name);
	  ch->kairank = 0;
	...
	if( !found )
	  if( !str_cmp(demonking,ch->name) )
	    found = TRUE;
	if( !found && ch->demonrank > 0 )
	{
	  bug("%s has a demon rank and is not on the global list. Removing rank.",ch->name);
          ch->demonrank = 0;
	  if( xIS_SET(ch->affected_by, AFF_SANCTUARY) )
            xREMOVE_BIT(ch->affected_by, AFF_SANCTUARY);
	}
...
//		ch->move += move_gain(ch);
	...
	if ( !IS_NPC(ch) && ch->level < LEVEL_IMMORTAL )
	{
		...
		if (ch->pcdata->bot_warn[0] >= 5)
		{
			...
			tIdle = ch->pcdata->pIdle[0] + ch->pcdata->pIdle[1] + ch->pcdata->pIdle[2] + ch->pcdata->pIdle[3] + ch->pcdata->pIdle[4];
			tIdle = tIdle / 5;
...
*/
	...
	if (!IS_NPC(ch))
	{
		...
		else if (xIS_SET(ch->act, PLR_PK1) || xIS_SET(ch->act, PLR_PK2)
				|| xIS_SET(ch->act, PLR_WAR2))
		{
			...
			else
			{
				...
				send_to_pager_color("&wYou can now turn off your PK flags.",ch);
			}
		}
      ...
      if( !IS_NPC( ch )
       ...
       && !IS_IMMORTAL( ch ) )
      {
        act( AT_ACTION, "Your time in the Hyperbolic Time Chamber is almost up!",
	...
	if( !IS_NPC(ch)	&& ch->pcdata->gnote_date > 0 && ch->pcdata->gnote_date <= current_time )
	{
      ...
      if ( !IS_NPC(ch) && !IS_IMMORTAL(ch)
      	&& ch->pcdata->release_date > 0 && ch->pcdata->release_date <= current_time)
      {
         ROOM_INDEX_DATA *location;
/*
         if ( ch->pcdata->clan )
...
*/
    ...
    if ( IS_AFFECTED(ch, AFF_SSJ) && IS_AFFECTED(ch, AFF_SSJ2)
		...
		break;
		case 2:
		act( AT_BLUE, "Electricity arcs around you.", ch, NULL, NULL, TO_CHAR    );
		act( AT_BLUE, "Electricity arcs around $n.", ch, NULL, NULL, TO_NOTVICT    );
		break;
                case 3:
    ...
    if ( IS_AFFECTED(ch, AFF_SSJ) && IS_AFFECTED(ch, AFF_SSJ2)
		...
		break;
		case 3:
		act( AT_YELLOW, "Your mane of hair shifts slightly, your aura pulsating with golden energy."
                   , ch, NULL, NULL, TO_CHAR    );
		act( AT_YELLOW, "$n's mane of hair shifts slightly, $s aura pulsating with golden energy."
    ...
    {
		act( AT_GREEN, "The green flames around you flare up brightly.", ch, NULL, NULL, TO_CHAR    );
		act( AT_GREEN, "The green flames around $n flare up brightly.", ch, NULL, NULL, TO_NOTVICT    );
    }
    ...
    if ( IS_AFFECTED(ch, AFF_KAIOKEN) )
    {
		...
		act( AT_RED, "A hum fills the room as the mystical flames around $n tingle your senses.", ch, NULL, NULL, TO_NOTVICT    );
    }
    ...
    {
		act( AT_WHITE, "You flex your bulging muscles, veins throbbing as pure energy courses through your body."
                   , ch, NULL, NULL, TO_CHAR    );
		act( AT_WHITE, "$n flexes $s bulging muscles, veins throbbing as pure energy courses through $s body."
                   , ch, NULL, NULL, TO_NOTVICT    );
    }
    ...
    {
		act( AT_PURPLE, "Your aura flashes purple, black lightning crackling before the energy melds back together."
                   , ch, NULL, NULL, TO_CHAR    );
		act( AT_PURPLE, "$n's aura flashes purple, black lightning crackling before the energy melds back together."
                   , ch, NULL, NULL, TO_NOTVICT    );
    }
    if ( IS_AFFECTED(ch, AFF_PERFECT) )
    {
		act( AT_DGREY, "The energy around you flickers and dances, the wailing of dead souls filling the air."
                   , ch, NULL, NULL, TO_CHAR    );
		act( AT_DGREY, "The energy around $n flickers and dances, the wailing of dead souls filling the air."
                   , ch, NULL, NULL, TO_NOTVICT    );
    }
...
*/
...

		{
		  if ( (is_saiyan(ch) || is_hb(ch))
			&& ((time_info.hour >= 20 && time_info.day == 29)
			|| (time_info.hour <=  5 && time_info.day == 0)) )
		  {
		    if( !xIS_SET(ch->affected_by, AFF_GOLDEN_OOZARU) &&
			 ch->pcdata->learned[gsn_ssj3] == 100 &&
			 is_saiyan(ch))
		  ...
		  if ( (is_saiyan(ch) || is_hb(ch))
			&& ((time_info.hour > 5 && time_info.day != 29)
			|| (time_info.hour <  20 && time_info.day != 0) ))
		  {
		    if ( xIS_SET(ch->affected_by, AFF_GOLDEN_OOZARU) )
		...
		{
	          if ( is_demon(ch)
                       && ((time_info.hour >= 20 && time_info.day == 14)
                       || (time_info.hour <=  5 && time_info.day == 15)) )
                  {
                    if ( !xIS_SET(ch->affected_by, AFF_MAKEOSTAR) &&
			  !wearing_chip(ch) )
                      transform_makeo(ch);
  	          ...
  	          if ( is_demon(ch)
                       && ((time_info.hour > 5 && time_info.day != 14)
                       || (time_info.hour <  20 && time_info.day != 15) ))
                  {
                    if ( xIS_SET(ch->affected_by, AFF_MAKEOSTAR) )
...
void hallucinations( CHAR_DATA *ch )
{
	...
	{
	    default:
	    case  1: t = "&gYou feel very restless... you can't sit still.\n\r";break;
	    case  2: t = "&gYou're tingling all over.\n\r";break;
	    //case  3: t = "Your skin is crawling.\n\r";break;
	    case  3: t = "&rYou're running around CoU while pkred at 1 pl trying to escape Sigma.\n\r";break;
	    case  4: t = "&rYou suddenly feel that something is terribly wrong.\n\r";	break;
	    case  5: t = "&RThose damn little fairies keep laughing at you!\n\r";		break;
	    case  6: t = "&gYou can hear your mother crying...\n\r";			break;
	    case  7: t = "&cHave you been here before, or not?  You're not sure...\n\r";	break;
	    case  8: t = "&rPainful childhood memories flash through your mind.\n\r";	break;
	    case  9: t = "&cYou hear someone call your name in the distance...\n\r";	break;
	    case 10: t = "&gYour head is pulsating... you can't think straight.\n\r";	break;
	    //case 11: t = "The ground... seems to be squirming...\n\r";break;
	    case 11: t = "&rSigma walks up to you and takes all your items and zeni.\n\r"; break;
	    case 12: t = "&gYou're not quite sure what is real anymore.\n\r";		break;
	    case 13: t = "&gIt's all a dream... or is it?\n\r";				break;
	    case 14: t = "&gYou hear your grandchildren praying for you to watch over them.\n\r";	break;
	    case 15: t = "&RThey're coming to get you... coming to take you away...\n\r";	break;
	    case 16: t = "&cYou begin to feel all powerful!\n\r";				break;
	    case 17: t = "&gYou're light as air... the heavens are yours for the taking.\n\r";	break;
	    case 18: t = "&gYour whole life flashes by... and your future...\n\r";	break;
	    case 19: t = "&gYou are everywhere and everything... you know all and are all!\n\r";	break;
	    case 20: t = "&gYou feel immortal!\n\r";					break;
	    case 21: t = "&gAhh... the power of a Supreme Entity... what to do...\n\r";	break;
	}
	ch_printf(ch,"&r%s",t);
	//send_to_char( t, ch );
    }
...
void update_alliances (void)
{
	...
	for (alliance = first_alliance; alliance; alliance = alliance->next)
	{
		...
		if (alliance->changeTimer <= 0)
		{
			...
			sprintf(buf, "%s has changed their alliance with your clan to %s", alliance->clan->name, clan_alliance_type[alliance->status]);
			echo_to_clan(alliance->vclan, buf);
			sprintf(buf, "Your clan has changed their alliance with %s to %s.", alliance->vclan->name, clan_alliance_type[alliance->status]);
...
/* Just commented out incase someone wants to use this - Goku
  static char *leetmsg[] =
  { "God shouts 'OH NOES! TEH END IZ NEER!1shift-one'",
    "OMIGAWD TEH LITENING GOING KRACK SNAPPLE POP LOLZ!!",
    "Crashes of thunder sound across the land.....IN MY PANTS!!",
    "teh sky iz black. wut?! UR RACEST! OMG SILENCE U!",
    "u c leif f0rms geting set up teh bomb!",
    "kold watur d00d.",
    "omg teh univurz iz un-table. hawtz0r",
    "u here skratch at ur wyndow. itz teh bogey man d00d!"
  };
*/
